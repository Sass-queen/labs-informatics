...
from sklearn.cluster import OPTICS, AgglomerativeClustering, DBSCAN
...
def apply_clustering(X, algorithm):
    ...
    if algorithm == "OPTICS":
        labels = OPTICS(min_samples=50, xi=0.05, min_cluster_size=0.05).fit_predict(X)
...
algorithms = ["OPTICS", "Agglomerative Clustering", "DBSCAN"]
...
for i, data_type in enumerate(data_types):
    for j, algo in enumerate(algorithms):
        ...
        ax.text(0.95, 0.05, f"{exec_time:.3f}s",
                ...
                bbox=dict(facecolor='orange', alpha=0.7, edgecolor='red'))
